...
class AkubelaScraper:
    ...
    def take_screenshot(self, page, name):
        """截图并记录文件名"""
        filename = f'data/{name}_{self.timestamp}.png'
        ...
        try:
            with sync_playwright() as p:
                ...
                try:
                    ...
                    self.take_screenshot(page, "01_after_login")
                ...
                try:
                    ...
                    self.take_screenshot(page, "02_after_env")
                except Exception as e:
            ...
            # ========== 步骤5: 保存数据（本地） ==========
            logger.info("【步骤5】保存数据到本地")
            self.save_data()

            # ========== 步骤6: 发送邮件（带截图） ==========
            logger.info("【步骤6】发送邮件（含截图）")
    ...
    def send_email(self, success=True):
        ...
        try:
            ...
            if success and self.data1_success and self.data2_success:
                status = "✅ 完全成功"
                subject = f'Akubela爬虫诊断截图 - {datetime.now().strftime("%Y-%m-%d %H:%M")}'
            elif success:
                status = "⚠️ 部分成功"
                subject = f'Akubela爬虫诊断截图（部分成功） - {datetime.now().strftime("%Y-%m-%d %H:%M")}'
            else:
                status = "❌ 失败"
                subject = f'Akubela爬虫诊断截图（失败） - {datetime.now().strftime("%Y-%m-%d %H:%M")}'
            ...
            <body>
                ...
                <h3>📸 截图列表（见附件）</h3>
                <ul>
                    {''.join([f'<li>{Path(f).name}</li>' for f in self.screenshot_files])}
                </ul>
                <p>请检查截图以定位问题。</p>
                <hr>
            ...
            for file_path in self.screenshot_files:
                if Path(file_path).exists():
                    with open(file_path, 'rb') as fp:
                        ...
                        msg.attach(part)
                        logger.info(f"  添加截图附件: {Path(file_path).name}")
                else:
